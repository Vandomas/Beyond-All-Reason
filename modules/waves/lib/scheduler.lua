--- Returns ORDERS, never effects. The gadget executes them and knows no policy;
--- that split is what lets the whole cadence run under busted with a fake world and a seeded RNG.
...
---Spec params are resolved once at Start; SetIntensity is runtime state a mission turns.
---Both multiply through HERE (size up, pace down) so when the policy fold ending lands,
---`policies/wave_scale.lua` has exactly one call site to absorb.
