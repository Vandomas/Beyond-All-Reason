--- Filters first and draws once where the monolith re-rolled up to a thousand times (rejection sampling that
--- silently spawned nothing); buckets carry an entry per weight point, so a uniform draw over the filtered list IS the weighted draw.
--- Output is the exact legacy queue-entry shape, so the ported drain and squad code consume it untouched.
