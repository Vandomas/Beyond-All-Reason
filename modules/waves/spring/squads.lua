--- A PORT, deliberately still inside waves: squads are born from the drain's batch boundaries and their zero-life
--- self-destruct is the anti-stalemate valve, so a module seam through the middle is the riskiest place to put one.
--- It reads plain squad state plus the behaviourOf/targetsOf hooks, never director internals, so it lifts out cheaply later.
