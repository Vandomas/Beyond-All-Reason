--- placement.lua takes Spring through deps, so the cascade can be driven without a game: the checks are
--- injected, and a fake that refuses anything demanding is exactly the map that broke it.
...
--- The DSL spec passed the whole time beacons arrived next to the player's base, because the retry path widened the named box until it covered the map.
