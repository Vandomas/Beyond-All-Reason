--- The preset does not invent a mode option; it SERIALIZES the scav_* wire keys a lobby and a
--- SPADS config already use, which is why turning the mode on in an existing lobby changes nothing.
...
M.Mode = ModeBuilder.Grammar({
	-- Scavengers is a way the game is played, so its preset lives on the game axis.
