...
-- Its own cadence, deliberately: this walks every capturable unit on the map, so it runs a
-- quarter of the roster per pass on a seven-frame beat out of phase with the director.
...
---Tougher units resist: the fourth root of the fourth root of health flattens the range between
---a solar and a fusion into something playable. Damaged units go much faster: the cube of the
---health fraction means a building at half health is taken eight times as quickly, so a raid
---that cannot finish a structure still costs you it.
...
function gadget:GameFrame(n)
	...
	-- convert() fires UnitGiven (and spawn effects UnitCreated) mid-walk, and
	-- inserting a key during next/pairs corrupts the iterator, so walk a
	-- snapshot and skip anything that left the set on the way.
	local ids = {}
	...
	for i = 1, #ids do
		...
		if capturable[unitID] and unitID % CAPTURE_PASSES == pass then
