--- Bootstraps the REAL mission runtime for a spec — the loader gadget, its
--- action files, the DSL/objectives/roster libraries and every required
--- module's mission_dsl contribution — over a Spring mock. Module apis are
--- recorders: a mission spec asserts the calls a mission makes into modules
--- that are validated in their own layer, never re-mocks their vocabulary.
...
function MissionBuilder:Load()
	...
	local wrapped = {
		...
		-- Placement is another module's contract; a spec of a mission only needs
		-- the spawn to land where it was asked.
