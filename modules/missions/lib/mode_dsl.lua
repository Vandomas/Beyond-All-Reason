--- matchflow owns the verdict, so a preset imports MatchFlow from its owner
--- directly (no re-export here): the preset names its true source.
...
M.Mode = ModeBuilder.Grammar({
	-- A mission is a way the game is played, so its preset lives on the game axis.
